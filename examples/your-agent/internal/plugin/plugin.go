...
func userManifestPath() string {
	if dir, err := os.UserConfigDir(); err == nil {
		return filepath.Join(dir, "your-agent", "plugins.json")
	}
	return ""
}

func projectManifestPath(cwd string) string {
	return filepath.Join(cwd, ".your-agent", "plugins.json")
}
...
func withMCPClient(ctx context.Context, server *MCPServerConfig, run func(*stdioClient) error) error {
	...
	defer client.close()
	var initialized any
	if err := client.call("initialize", map[string]any{"protocolVersion": "2024-11-05", "capabilities": map[string]any{}, "clientInfo": map[string]any{"name": "your-agent", "version": "1"}}, &initialized); err != nil {

...
func TestPluginAndMCPToolsAreDiscoveredAndExecuted(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, ".your-agent")
	...
	manifest := map[string]any{
		...
		"plugins": []any{map[string]any{
			...
			"env": map[string]string{"YOUR_AGENT_HELPER": "1"}, "input_schema": map[string]any{"type": "object", "properties": map[string]any{"value": map[string]string{"type": "string"}}, "required": []string{"value"}},
		}},
		"mcp_servers": []any{map[string]any{
			"name": "local", "command": []string{os.Args[0], "-test.run=TestExternalToolHelper", "--", "mcp"}, "env": map[string]string{"YOUR_AGENT_HELPER": "1"},
		}},
	}
...
func TestExternalToolHelper(t *testing.T) {
	if os.Getenv("YOUR_AGENT_HELPER") != "1" {
		return
	}

...
func (s *Store) maybePrewarmSummary(ctx context.Context, sessionID string) {
	...
	predictedText := "[next user turn]"
	predicted := append(cloneMessages(messages), Message{
		SessionID: sessionID,
		Role:      "user",
		Content:   predictedText,
		Blocks:    []domain.ContentBlock{{Type: domain.BlockText, Text: predictedText}},
	})
...
func sessionSummaryEnabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("YOUR_AGENT_SESSION_LLM_SUMMARY"))) {
	case "0", "false", "off", "no":
		return false
	default:
		return true

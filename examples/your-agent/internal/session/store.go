...
type Info struct {
	...
	DedupeFoldedResults int    `json:"dedupeFoldedResults"`
	DedupeBytesSaved    int    `json:"dedupeBytesSaved"`
...
type Status struct {
	...
	TotalToolFailures   int       `json:"totalToolFailures"`
...
CREATE INDEX IF NOT EXISTS idx_session_turns_session ON session_turns(session_id, committed_at, id);
...
CREATE INDEX IF NOT EXISTS idx_session_turn_events_session ON session_turn_events(session_id, sequence);
CREATE TABLE IF NOT EXISTS session_turn_metrics (
	...
	human_approval_requests INTEGER NOT NULL DEFAULT 0,
	context_compactions INTEGER NOT NULL DEFAULT 0,
	success INTEGER NOT NULL DEFAULT 0,
	FOREIGN KEY(turn_id) REFERENCES session_turns(id) ON DELETE CASCADE,
	FOREIGN KEY(session_id) REFERENCES sessions(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_session_turn_metrics_session ON session_turn_metrics(session_id, completed_at, turn_id);`
	...
	for name, definition := range map[string]string{
		...
		if err := s.ensureColumn("session_messages", name, definition); err != nil {
			return err
		}
	}
	if err := s.backfillLegacyTurns(context.Background()); err != nil {
		return fmt.Errorf("backfill legacy session turns: %w", err)
	}
	return nil
}

func (s *Store) ensureSessionColumn(name, definition string) error {
	return s.ensureColumn("sessions", name, definition)
}
...
func (s *Store) AppendTurn(ctx context.Context, sessionID, userText, assistantText string, inputTokens int) error {
	turn, err := s.BeginTurn(ctx, sessionID, "")
	if err != nil {
		return err
	}
	if err := turn.AddUser(userText, nil); err != nil {
		return err
	}
	if err := turn.AddAssistantText(assistantText); err != nil {
		return err
	}
	metrics := domain.RunMetrics{InputTokens: max(inputTokens, 0), TotalTokens: max(inputTokens, 0), Success: true}
	if err := turn.SetMetrics(metrics); err != nil {
		return err
	}
	return turn.Commit(ctx, TurnCompleted, "")
}
...
func (s *Store) BuildPrompt(ctx context.Context, sessionID, current string) (View, error) {
	return s.BuildPromptWithImages(ctx, sessionID, current, nil)
}

func (s *Store) BuildPromptWithImages(ctx context.Context, sessionID, current string, images []string) (View, error) {
	return s.buildPrompt(ctx, sessionID, current, images, false, s.config.MinRecentTurns)
}

func (s *Store) ForcePrompt(ctx context.Context, sessionID, current string, minRecentTurns int) (View, error) {
	return s.ForcePromptWithImages(ctx, sessionID, current, nil, minRecentTurns)
}

func (s *Store) ForcePromptWithImages(ctx context.Context, sessionID, current string, images []string, minRecentTurns int) (View, error) {
	if minRecentTurns < 0 {
		minRecentTurns = 0
	}
	return s.buildPrompt(ctx, sessionID, current, images, true, minRecentTurns)
}

func (s *Store) buildPrompt(ctx context.Context, sessionID, current string, images []string, force bool, minRecentTurns int) (View, error) {
	sessionID, err := s.Ensure(ctx, sessionID)
	if err != nil {
		return View{}, err
	}
	...
	var currentBlocks []domain.ContentBlock
	if strings.TrimSpace(current) != "" {
		currentBlocks = append(currentBlocks, domain.ContentBlock{Type: domain.BlockText, Text: current})
	}
	for _, image := range images {
		currentBlocks = append(currentBlocks, domain.ContentBlock{Type: domain.BlockImage, ImageURL: image})
	}
	messages = append(messages, Message{SessionID: sessionID, Role: "user", Content: current, Blocks: currentBlocks, CreatedAt: s.now().UTC()})
	bytesBefore := totalBytes(messages)
	promptMessages := cloneMessages(messages)
	microCount := microCompact(promptMessages)
	dedupeInfo := dedupeStaleResults(promptMessages)
	bytesAfterHousekeeping := totalBytes(promptMessages)
	lastTokens, err := s.lastInputTokens(ctx, sessionID)
	if err != nil {
		return View{}, err
	}
	triggeredBy := ""
	if bytesAfterHousekeeping >= s.config.TriggerBytes {
		triggeredBy = "bytes"
	}
	...
	info := Info{
		...
		MicroCompactions: microCount, DedupeFoldedResults: dedupeInfo.FoldedResults,
		DedupeBytesSaved: dedupeInfo.BytesSaved, RetainedTurns: countUserTurns(promptMessages),
	}
	if triggeredBy != "" {
	...
	promptMessages = repairStructuredHistory(promptMessages)
	...
	history := cloneMessages(promptMessages)
	if len(history) > 0 {
		history = history[:len(history)-1]
	}
	return View{SessionID: sessionID, Prompt: prompt, Messages: promptMessages, History: history, Info: info}, nil
}

func (s *Store) Messages(ctx context.Context, sessionID string) ([]Message, error) {
	rows, err := s.db.QueryContext(ctx, `
SELECT id, session_id, turn_id, sequence, role, content, blocks_json, created_at
FROM session_messages WHERE session_id = ? ORDER BY id`, strings.TrimSpace(sessionID))
	...
	for rows.Next() {
		var message Message
		var created, blocksJSON string
		if err := rows.Scan(&message.ID, &message.SessionID, &message.TurnID, &message.Sequence, &message.Role, &message.Content, &blocksJSON, &created); err != nil {
		...
		if strings.TrimSpace(blocksJSON) != "" {
			_ = json.Unmarshal([]byte(blocksJSON), &message.Blocks)
		}
		if len(message.Blocks) == 0 && strings.TrimSpace(message.Content) != "" {
			message.Blocks = []domain.ContentBlock{{Type: domain.BlockText, Text: message.Content}}
		}
...
SELECT s.id, s.title, s.forked_from_id, s.last_input_tokens,
       s.summary_calls, s.summary_input_tokens, s.summary_output_tokens, s.summary_latency_ms,
	       ...
	       (SELECT COUNT(*) FROM session_turn_events e WHERE e.session_id = s.id),
	       (SELECT COUNT(*) FROM session_turns t WHERE t.session_id = s.id),
	       COALESCE((SELECT t.status FROM session_turns t WHERE t.session_id = s.id ORDER BY t.committed_at DESC, t.id DESC LIMIT 1), ''),
	       COALESCE((SELECT SUM(tm.input_tokens) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.output_tokens) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.total_tokens) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.tool_calls) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.tool_failures) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.duration_ms) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0),
	       COALESCE((SELECT SUM(tm.success) FROM session_turn_metrics tm WHERE tm.session_id = s.id), 0)
...
WHERE s.id = ? GROUP BY s.id`, strings.TrimSpace(sessionID)).Scan(
		...
		&created, &updated, &status.MessageCount, &status.EventCount, &status.CanonicalEventCount,
		&status.TurnCount, &status.LastTurnStatus, &status.TotalInputTokens, &status.TotalOutputTokens,
		&status.TotalTokens, &status.TotalToolCalls, &status.TotalToolFailures, &status.TotalDurationMS, &status.SuccessfulTurns,
	)
	if err != nil {
...
SELECT ?, '', 0, role, content, blocks_json, created_at FROM session_messages WHERE session_id = ? ORDER BY id`, newID, sourceID); err != nil {
...
SELECT ?, run_id, event_type, payload_json, created_at FROM session_events WHERE session_id = ? ORDER BY id`, newID, sourceID); err != nil {
	...
	if err := s.backfillLegacyTurns(ctx); err != nil {
		return "", fmt.Errorf("materialize forked session turns: %w", err)
	}
	return newID, nil
}
...
func totalBytes(messages []Message) int {
	...
	for _, message := range messages {
		...
		if len(message.Blocks) > 0 {
			if data, err := json.Marshal(message.Blocks); err == nil {
				total += len(data)
			}
		}
	}
...
func cloneMessages(messages []Message) []Message {
	...
	for index, message := range messages {
		...
		result[index].Blocks = cloneBlocks(message.Blocks)
	}
	return result
}
...
func microCompact(messages []Message) int {
	...
	lastToolResult := lastToolResultMessageIndex(messages)
	for index := range messages {
		message := &messages[index]
		if index < len(messages)-2 && plainTextOnly(message.Blocks) {
			key := message.Role + "\x00" + message.Content + "\x00" + blocksKey(message.Blocks)
		...
		if index < len(messages)-2 {
			changed := compactBlocks(message.Blocks, index != lastToolResult)
			if changed > 0 {
				message.Content = displayText(message.Blocks)
				compacted += changed
			} else if len(message.Content) > maxPromptMessageBytes {
				message.Content = compactText(message.Content, message.Role)
				message.Blocks = []domain.ContentBlock{{Type: domain.BlockText, Text: message.Content}}
				compacted++
			}
		}
...
func findCutIndex(messages []Message, minRecentTurns int) int {
	...
	for index := len(messages) - 1; index > 0; index-- {
		if !plainUserMessage(messages[index]) {
			continue
		}
		seen++
...
func plainUserMessage(message Message) bool {
	if message.Role != "user" || len(message.Blocks) == 0 {
		return false
	}
	for _, block := range message.Blocks {
		if block.Type != domain.BlockText && block.Type != domain.BlockImage {
			return false
		}
	}
	return true

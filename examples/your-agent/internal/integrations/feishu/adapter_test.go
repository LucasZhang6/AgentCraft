...
func TestAdapterCloseCancelsAndWaitsForPolling(t *testing.T) {
	...
	yourAgentSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
	...
	adapter, err := NewAdapter(Config{
		FeishuBaseURL: feishuSrv.URL, YourAgentBaseURL: yourAgentSrv.URL,
...
func TestCallbackEndpointRejectsMissingOrInvalidConfiguredToken(t *testing.T) {
	var yourAgentCalled atomic.Bool
	yourAgentSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		yourAgentCalled.Store(true)
	...
	adapter, err := NewAdapter(Config{
		...
		YourAgentBaseURL:  yourAgentSrv.URL,
	...
	if yourAgentCalled.Load() {
		t.Fatal("unauthenticated callback reached the YourAgent API")
	}
}

func TestAdapterMessageSubmitsYourAgentTaskAndReplies(t *testing.T) {
	var executeSeen atomic.Bool
	var replySeen atomic.Bool

	yourAgentSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/agent/execute" {
			t.Fatalf("unexpected yourAgent path: %s", r.URL.Path)
		}
...
func TestAdapterDownloadsFeishuImageAndForwardsItToYourAgent(t *testing.T) {
	var executeSeen atomic.Bool
	...
	if !executeSeen.Load() {
		t.Fatal("YourAgent execute endpoint was not called")
	}
}
...
func TestFormatFinalNormalizesPlainTextFallback(t *testing.T) {
	...
	for _, want := range []string{"YourAgent 任务完成：", "天气：多云转中雨", "气温：约 25-31°C", "结论：今天不太适合安排长时间户外游玩。", "• 带伞或轻便雨衣；"} {
...
func TestAdapterIgnoresGroupMessageMentioningEveryone(t *testing.T) {
	...
	if executeSeen.Load() {
		t.Fatal("@all message should not execute a YourAgent task")
	}
}
...
func TestAdapterAcceptsGroupMessageWithExplicitMention(t *testing.T) {
	...
	if !executeSeen.Load() {
		t.Fatal("explicit bot mention should execute a YourAgent task")
	}
}
...
func TestAdapterApprovalActionUpdatesCard(t *testing.T) {
	...
	if !approveSeen.Load() {
		t.Fatal("YourAgent approve request was not sent")
	}
...
func TestAdapterApproveAllAction(t *testing.T) {
	...
	adapter, err := NewAdapter(Config{
		FeishuBaseURL:    yourAgentSrv.URL,
		YourAgentBaseURL: yourAgentSrv.URL,
		AppID:            "cli_test",
		AppSecret:        "secret",
		DBPath:           t.TempDir() + "/feishu.db",

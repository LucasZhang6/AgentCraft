...
func TestParseCallbackMessageEvent(t *testing.T) {
	body := []byte(`{
		...
		"event":{
			...
			"message":{"message_id":"om_1","root_id":"om_root","parent_id":"om_parent","chat_id":"oc_1","chat_type":"p2p","message_type":"text","content":"{\"text\":\"hello yourAgent\"}","mentions":[{"key":"@_user_1","id":{"open_id":"ou_bot"},"name":"ShadowSRE"},{"key":"@_user_2","id":{"open_id":"all"},"name":"所有人"}]}
		}
	}`)
	...
	if cb.Message.Text != "hello yourAgent" || cb.Message.ChatID != "oc_1" || cb.Message.SenderOpenID != "ou_1" {

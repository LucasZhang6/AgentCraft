...
func TestLiveSessionSummarizer(t *testing.T) {
	if os.Getenv("YOUR_AGENT_LIVE_TEST") != "1" {
		t.Skip("set YOUR_AGENT_LIVE_TEST=1 to call the configured Responses API")
	}
	...
	client := provider.NewOpenAIClient(provider.OpenAIConfig{
		...
		UserAgent: "your-agent-live-test", MaxRetries: 1, RequestTimeout: 90 * time.Second,
	})

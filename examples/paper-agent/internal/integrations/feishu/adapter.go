...
func NewAdapter(cfg Config, logger *log.Logger) (*Adapter, error) {
	...
	return &Adapter{
		cfg:        cfg,
		store:      store,
		paperAgent: NewPaperAgentClient(cfg.PaperAgentBaseURL, cfg.PaperAgentAccessID),
...
func (a *Adapter) handleMessage(ctx context.Context, msg MessageEvent) {
	...
	case isCommand(text, "/new"):
		...
		a.replyText(ctx, msg.MessageID, "已为这个飞书会话开启新的 PaperAgent session。")
		return
	...
	messageForPaperAgent, images, err := a.inputForPaperAgent(ctx, msg, text)
	if err != nil {
		a.replyText(ctx, msg.MessageID, "读取飞书图片失败："+err.Error())
		return
	}
	exec, err := a.paperAgent.Execute(ctx, ExecuteRequest{
		Message:     messageForPaperAgent,
		Images:      images,
		Mode:        a.cfg.Mode,
	...
	if err != nil {
		a.replyText(ctx, msg.MessageID, "提交 PaperAgent 任务失败："+err.Error())
		return
	}
	...
	a.replyTextInThread(ctx, msg.MessageID, fmt.Sprintf("已提交 PaperAgent 任务：%s\n输入 /status 查看进度，/cancel 取消。", exec.TaskID))
...
func (a *Adapter) inputForPaperAgent(ctx context.Context, msg MessageEvent, text string) (string, []string, error) {
...
func (a *Adapter) replyStatus(ctx context.Context, msg MessageEvent, key string) {
	...
	if err != nil || mapping == nil || mapping.LastTaskID == "" {
		a.replyText(ctx, msg.MessageID, "当前飞书会话没有正在跟踪的 PaperAgent 任务。")
		return
	}
	status, err := a.paperAgent.Status(ctx, mapping.LastTaskID)
	if err != nil {
...
func (a *Adapter) cancelLastTask(ctx context.Context, msg MessageEvent, key string) {
	...
	if err := a.paperAgent.Cancel(ctx, mapping.LastTaskID); err != nil {
...
func (a *Adapter) handleAction(ctx context.Context, action ActionEvent) error {
	...
	case "approve":
		return a.paperAgent.Approve(ctx, action.TaskID, action.ToolID, true, "")
	case "approve_all":
		return a.paperAgent.ApproveAll(ctx, action.TaskID, action.ToolID, "Approved remaining operations for this task from Feishu")
	case "reject", "deny":
		output := action.Output
		if output == "" {
			output = "Rejected from Feishu"
		}
		return a.paperAgent.Approve(ctx, action.TaskID, action.ToolID, false, output)
	default:
...
func (a *Adapter) pollTask(ctx context.Context, chatID, replyToMessageID, taskID string) {
	...
	for {
		...
		case <-ctx.Done():
			...
			message := "PaperAgent 任务轮询超时：" + taskID
			a.replyCardToChat(context.Background(), chatID, resultCard("PaperAgent 任务超时", "orange", message), message)
			return
		case <-ticker.C:
		}
		status, err := a.paperAgent.Status(ctx, taskID)
		if err != nil {
...
func finalResultCard(status StatusResponse) map[string]any {
	...
	if !status.Success {
		...
		return resultCard("PaperAgent 任务失败", "red", content)
	}
	...
	return resultCard("PaperAgent 任务完成", "green", content)
}
...
func trimForFeishu(text string) string {
	...
	return text[:max-80] + "\n\n[输出过长，已截断；请在 Paper Agent session 中查看完整内容]"
}

...
func RegisterPaperTools(registry *Registry, papers []domain.Paper) error {
	if err := registry.Register(Definition{
		Name:        "search_papers",
		Description: "Search the local, curated paper catalog by topic.",
		Risk:        "read-only",
	...
	return registry.Register(Definition{
		Name:        "read_paper_card",
		Description: "Read a curated structured card from the local paper catalog.",
		Risk:        "read-only",
		Schema: domain.ToolSchema{
			Type:       "object",
			Required:   []string{"id"},

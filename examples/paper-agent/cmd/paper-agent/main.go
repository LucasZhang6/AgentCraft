...
import (
	...
	inputreader "github.com/LucasZhang6/AgentCraft/examples/paper-agent/internal/input"
	memorystore "github.com/LucasZhang6/AgentCraft/examples/paper-agent/internal/memory"
	...
	"github.com/LucasZhang6/AgentCraft/examples/paper-agent/internal/tui"
	"github.com/LucasZhang6/AgentCraft/examples/paper-agent/internal/ui"
)
...
func parseFlags() config {
	...
	flag.StringVar(&cfg.provider, "provider", envOr("PAPER_AGENT_PROVIDER", "demo"), "model provider: demo or openai")
	...
	flag.StringVar(&cfg.sessionID, "session-id", os.Getenv("PAPER_AGENT_SESSION_ID"), "reuse a persisted session")
...
func runInteractive(cfg config, store *session.Store, goals *goalstore.Store, terminal *ui.UI) error {
	...
	for {
		...
		case lower == "/plan" || strings.HasPrefix(lower, "/plan "):
			resumePrompt, shouldRun := handlePlanCommand(context.Background(), terminal, cfg.dataDir, sessionID, line)
			if !shouldRun {
				continue
			}
			line = resumePrompt
		...
		ctx, cancel := signal.NotifyContext(context.Background(), os.Interrupt)
		stopSpinner := terminal.Spinner("Paper Agent 正在处理")
		output, err := executeTurn(ctx, cfg, store, sessionID, line, terminal, stopSpinner, nil)
		stopSpinner()
		cancel()
		if err != nil {
...
func printSessionStatus(ctx context.Context, terminal *ui.UI, store *session.Store, sessionID string) {
	...
	terminal.PrintInfo("session=%s | title=%s | messages=%d | last_input_tokens=%d | updated=%s",
		status.SessionID, title, status.MessageCount, status.LastInputTokens, status.UpdatedAt.Local().Format(time.RFC3339))
...
func handleSessionCommand(ctx context.Context, terminal *ui.UI, store *session.Store, sessionID, command string) string {
	...
	case "list", "ls":
		...
		for _, item := range items {
			...
			terminal.Println("%s %s | %s | %d messages | %s", marker, item.SessionID, title, item.MessageCount, item.UpdatedAt.Local().Format("2006-01-02 15:04"))
		}
	case "save":
		if len(parts) < 3 {
...
func handlePlanCommand(ctx context.Context, terminal *ui.UI, dataDir, sessionID, command string) (string, bool) {
	...
	case "resume":
		...
		encoded, _ := json.Marshal(item.Steps)
		return fmt.Sprintf("Resume the persisted plan without repeating completed steps. Objective: %s\nPlan ID: %s\nPersisted steps: %s\nVerify dependency order and preserve evidence.", item.Objective, item.ID, encoded), true
	case "accept":
		if len(parts) < 4 {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		runtime, err := app.New(app.Config{
			...
			OnEvent: func(event domain.Event) {
				_ = store.AppendEvent(context.Background(), sessionID, activeRunID, event)
				printer(event)
			},
		...
		result, err := runtime.Agent.Run(ctx, view.Prompt)
		...
		if err == nil {
			...
			_ = runtime.Close()
			if err := store.AppendTurn(ctx, sessionID, input, result.Report, result.Metrics.InputTokens); err != nil {
				return turnOutput{}, err
			}
			return output, nil
		}

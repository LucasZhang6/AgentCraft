...
import (
	...
	"github.com/LucasZhang6/AgentCraft/examples/paper-agent/internal/integrations/feishu"
)
...
func main() {
	...
	flag.StringVar(&cfg.PaperAgentBaseURL, "paper-agent-url", cfg.PaperAgentBaseURL, "Paper Agent HTTP API base URL")
	flag.StringVar(&cfg.PaperAgentAccessID, "paper-agent-access-id", cfg.PaperAgentAccessID, "Paper Agent access id used to obtain a bearer token")
	flag.StringVar(&cfg.DBPath, "db", cfg.DBPath, "adapter SQLite database path")
	flag.StringVar(&cfg.Mode, "mode", cfg.Mode, "Paper Agent mode: chat, agent, or plan")
	flag.DurationVar(&cfg.PollInterval, "poll-interval", cfg.PollInterval, "Paper Agent task polling interval")
	flag.DurationVar(&cfg.PollTimeout, "poll-timeout", cfg.PollTimeout, "Paper Agent task polling timeout")
	...
	logger.Printf("listening on %s, webhook path /feishu/events, paper-agent=%s", server.Addr, resolved.PaperAgentBaseURL)

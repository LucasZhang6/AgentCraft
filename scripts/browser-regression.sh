...
TMP_DIR="$(mktemp -d)"
AGENT_PORT="${YOUR_AGENT_BROWSER_PORT:-18090}"
...
"$ROOT/dist/bin/your-agent-server" -provider demo -addr "127.0.0.1:$AGENT_PORT" -data-dir "$TMP_DIR/data" -work-dir "$TMP_DIR/workspace" >"$TMP_DIR/server.log" 2>&1 &
AGENT_SERVER_PID=$!
for _ in {1..50}; do
...
cd "$ROOT"
YOUR_AGENT_WEB_URL="http://127.0.0.1:$AGENT_PORT" node scripts/webui-regression.mjs

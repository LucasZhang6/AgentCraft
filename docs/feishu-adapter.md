...
Your Agent HTTP API. It owns Feishu credentials, callback verification, card
...
callback payloads are not implemented yet. Your Agent also does not copy the
...
The default address is `127.0.0.1:18080`. `YOUR_AGENT_ACCESS_ID` enables a
...
| `--your-agent-url` | `YOUR_AGENT_BASE_URL` | `http://127.0.0.1:18080` | Your Agent API |
| `--your-agent-access-id` | `YOUR_AGENT_ACCESS_ID` | | Agent login credential |
| `--db` | `FEISHU_ADAPTER_DB` | `~/.config/your-agent/feishu-adapter.db` | mapping SQLite database |
...
| `--auto-approve` | `YOUR_AGENT_FEISHU_AUTO_APPROVE` | `false` | trusted automation only |
...
or tunnel. Keep Your Agent Server on loopback when both processes share a host.
...
the final permission decision remain in Your Agent Runtime. Long replies are

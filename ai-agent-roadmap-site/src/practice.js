const shared = {
  name: "Paper Agent",
  repoPath: "examples/paper-agent"
};

const english = {
  ...shared,
  subtitle: "A durable Go agent runtime that keeps model proposals inside host-owned boundaries",
  summary:
    "Paper Agent is the executable counterpart of this research map. It combines a deterministic offline model with an OpenAI-compatible streaming provider, then places both behind the same controller, tool policy, persisted Session and Goal state, recoverable planning, lifecycle memory, evaluation, metrics, and user surfaces. The result is a reference runtime whose success and failure paths can be replayed rather than a prompt demo.",
  stats: [
    { label: "Persistent stores", value: "5", note: "Session, Goal, Plan, Memory, and Metrics in SQLite" },
    { label: "Built-in tools", value: "14", note: "paper, file, shell, web, clarification, and subagent capabilities" },
  ...
  phases: [
    {
      order: "01",
      date: "Provider and decisions",
      title: "Make model output observable and replaceable",
      commits: "Provider + Model",
      gitSignal:
        "internal/provider/openai.go owns Responses HTTP/SSE, fallback, retry, prompt-cache and usage parsing. internal/model/openai.go parses candidate plans and actions; internal/model/demo.go preserves deterministic offline behavior.",
      built:
        "The same controller can run with a zero-network DemoModel or a real OpenAI-compatible model. Final answer tokens stream to the UI, while controller JSON remains internal and API credentials stay in environment variables.",
      theory:
      ...
      practice:
        "Keep transport, model policy, and host execution as separate interfaces. Test SSE fragments, usage, cache fields, transient failures, fallback order, malformed JSON, and gateway compatibility without granting a model direct tool access."
    },
    {
      order: "02",
      date: "Agent and Goal loops",
      title: "Separate local reasoning from long-running continuation",
      commits: "Controller + Goal",
      gitSignal:
        "internal/agent/agent.go runs bounded Decide-Tool-Observation steps. internal/goal/store.go persists objective, state, tokens, turns, auto-resume, and pause/resume/clear transitions independently from chat messages.",
      built:
        "A Goal can continue across turns and process restarts when evaluation has not accepted the result. Zero token and Goal-turn limits mean unlimited by default, while every inner turn still has max-steps, cancellation, and explicit stop reasons.",
      theory:
      ...
      practice:
        "Give each loop a clear owner and budget. Persist the Goal identity and transitions, return tool failures as observations, and test cancellation, pause, evaluator retry, unlimited defaults, and explicit budget exhaustion."
    },
    {
      order: "03",
      date: "Tool authority",
      ...
      gitSignal:
        "internal/tools registers paper, file, shell, glob/grep, web, clarification, and subagent capabilities. internal/plugin loads command plugins and MCP stdio servers through the same registry.",
      built:
    ...
    },
    {
      ...
      gitSignal:
        "internal/session stores messages and structured events in SQLite, supports status/list/title/fork, and implements L1 micro-compaction, L2 digest, asynchronous L3 summary prewarm, usage accounting, and context-length recovery.",
      built:
        "Complete messages remain available after restart. At a byte or token threshold, the prompt keeps recent turns and a digest; OpenAI mode may add a ready semantic summary. Failure or a 12-second timeout falls back without blocking the user request.",
      theory:
      ...
      practice:
        "Store the source of truth outside context, compact on user or subgoal boundaries, record summary coverage and cost, and provide deterministic fallback. Test full-history retention, stale summary rejection, async failure, and two-stage provider recovery."
    },
    {
      order: "05",
      ...
      built:
        "Model-generated plans survive restart, independent roles can run concurrently, and steps with human checks wait in awaiting_acceptance. CLI and HTTP can inspect, resume, and accept the same persisted plan.",
      theory:
    ...
    },
    {
      ...
      gitSignal:
        "cmd and internal/ui/input/render/tui/server/integrations expose readline, Markdown, full-screen TUI, asynchronous HTTP/SSE, Web UI, and Feishu. Make and CI provide E2E, browser, open-source, cross-platform archive, checksum, and signing paths.",
      built:
      ...
      theory:
        "Agent engineering is a whole-system property. Trajectory and RL work require structured traces, while efficiency work requires cost at the same boundary as success. A unit-test-only controller does not prove a usable interface.",
      practice:
  ...
  theoryMaps: [
    { theory: "LLM foundations", implementation: "OpenAI-compatible and deterministic providers", files: ["internal/provider/openai.go", "internal/model/"], explanation: "In-context behavior and alignment live behind provider/model interfaces. Streaming, fallback, cache and usage are transport facts; plans and actions remain proposals parsed into local contracts." },
    { theory: "Agent Loop", implementation: "Bounded ReAct plus persisted Goal continuation", files: ["internal/agent/agent.go", "internal/goal/store.go"], explanation: "One turn alternates decision and observation; the outer Goal continues after failed acceptance. Cancellation, pause, budget, and stop reasons remain explicit." },
  ...
  codeTour: [
    { area: "Start at runtime composition", paths: ["cmd/paper-agent/main.go", "internal/app/factory.go"], read: "Follow flags and environment into provider, SQLite stores, registry, evaluator, metrics, UI, and lifecycle cleanup." },
    { area: "Trace model transport and parsing", paths: ["internal/provider/openai.go", "internal/model/openai.go"], read: "Separate SSE/retry/fallback/cache handling from plan and action JSON contracts and their error paths." },
    { area: "Follow one Goal turn", paths: ["internal/agent/agent.go", "internal/goal/store.go"], read: "Observe how decisions, tools, observations, evaluation, budgets, and persisted Goal transitions interact." },
...
const chinese = {
  ...shared,
  subtitle: "由宿主掌握权限、可持久恢复的 Go Agent Runtime",
  summary:
    "Paper Agent 是研究图谱的可执行对应物。它把确定性离线模型与 OpenAI-compatible 流式 Provider 放在同一套 Controller、工具策略、Session、Goal、Planning、Memory、Evaluation、Metrics 和 UI 边界中。结果不是一段 Prompt Demo，而是一套成功和失败都能回放的参考 Runtime。",
  stats: [
    { label: "持久存储", value: "5", note: "Session、Goal、Plan、Memory、Metrics 均使用 SQLite" },
    { label: "内置工具", value: "14", note: "论文、文件、Shell、网页、澄清和子 Agent" },
  ...
  phases: [
    {
      order: "01", date: "Provider 与决策", title: "让模型输出可观察、可替换", commits: "Provider + Model",
      gitSignal: "internal/provider/openai.go 负责 Responses HTTP/SSE、回退、重试、Prompt Cache 和 Usage；internal/model/openai.go 解析计划与动作，demo.go 保留确定性离线路径。",
      built: "同一个 Controller 可连接 DemoModel 或真实 OpenAI-compatible 模型。最终回答可流式进入 UI，Controller JSON 留在内部，API 凭据只来自环境变量。",
      theory: "Few-shot 与结构化 Prompt 提供临时适配，对齐研究改善默认行为，但都不会把模型输出变成可信执行契约。",
      practice: "把传输、模型策略和宿主执行拆成接口，测试 SSE、Usage、Cache、瞬时错误、回退顺序、非法 JSON 和兼容网关。"
    },
    {
      order: "02", date: "Agent 与 Goal Loop", title: "区分局部推理与长任务 continuation", commits: "Controller + Goal",
      gitSignal: "internal/agent/agent.go 执行有界 Decide-Tool-Observation；internal/goal/store.go 独立保存目标、状态、Token、轮次、自动恢复和 pause/resume/clear。",
      built: "验收未通过时，同一 Goal 可跨轮和重启继续。Token 与 Goal turn 默认零表示不限，每个内层 turn 仍有 max-steps、取消和停止原因。",
      theory: "Agent Loop 把完成调用变成环境反馈；高效 Agent 要联合衡量结果和成本；长任务需要持久目标，而不是反复改写 Prompt。",
      practice: "为两层循环定义所有者和预算，持久化 Goal 身份与转移，把工具失败作为 Observation，并测试取消、暂停、重试和预算耗尽。"
    },
    {
      order: "03", date: "工具权限", title: "把真实能力放进统一策略 Registry", commits: "Tools + Plugins + MCP",
      gitSignal: "internal/tools 注册论文、文件、Shell、glob/grep、网页、澄清和子 Agent；internal/plugin 通过同一 Registry 加载命令插件和 MCP stdio。",
    ...
    {
      order: "04", date: "持久状态与上下文", title: "保留全量历史，只压缩送模视图", commits: "Session + Compaction",
      gitSignal: "internal/session 用 SQLite 保存消息和事件，支持状态/列表/标题/Fork，并实现 L1、L2、异步 L3、用量统计和 context-length 恢复。",
      built: "完整消息跨重启保留。达到阈值后送模视图保留近期轮次和 digest；OpenAI 模式可加入已准备好的语义摘要，失败或 12 秒超时不阻塞请求。",
      theory: "Transformer 的上下文成本与 HiAgent 的层级工作记忆共同说明需要选择性视图；压缩不能等于静默删除。",
      practice: "把事实源放在上下文外，在用户轮次或子目标边界压缩，记录摘要覆盖范围和成本，并测试全量保留、摘要失败和两级恢复。"
    },
    {
      ...
      built: "模型生成计划可跨重启恢复，独立角色可并行；需要人工检查的步骤进入 awaiting_acceptance，CLI 与 HTTP 可查看、继续和验收。",
    ...
    },
    {
      ...
      gitSignal: "cmd 与 internal/ui/input/render/tui/server/integrations 提供 readline、Markdown、TUI、HTTP/SSE、Web 和飞书；Make/CI 提供 E2E、浏览器、开源、跨平台归档、校验和与签名。",
      built: "用户可从终端、浏览器或飞书继续同一状态并取消/审批。脱敏 JSONL 与 SQLite 指标可跨 Run 比较 Provider、工具、压缩和结果。",
      theory: "Agent 工程化是整体属性。轨迹/RL 需要结构化 Trace，高效 Agent 需要在成功边界记录成本。只测 Controller 单元函数不足以证明可用。",
      practice: "所有 Adapter 复用一个核心，测试真实进程和浏览器可见状态，在每个入口保持权限边界，只发布可复现、带校验和并说明局限的产物。"
    }
  ],
  theoryMaps: [
    { theory: "LLM 基础", implementation: "OpenAI-compatible 与确定性 Provider", files: ["internal/provider/openai.go", "internal/model/"], explanation: "流式、回退、Cache 和 Usage 属于 Provider；计划和动作仍是不可信提议。" },
    { theory: "Agent Loop", implementation: "有界 ReAct 与持久 Goal continuation", files: ["internal/agent/agent.go", "internal/goal/store.go"], explanation: "单轮交替决策与观察，外层在验收失败后继续；取消、暂停和预算都显式化。" },
  ...
  codeTour: [
    { area: "从 Runtime 装配开始", paths: ["cmd/paper-agent/main.go", "internal/app/factory.go"], read: "沿 Flag 和环境变量查看 Provider、SQLite、Registry、Evaluator、Metrics、UI 和资源关闭。" },
    { area: "追踪模型传输与解析", paths: ["internal/provider/openai.go", "internal/model/openai.go"], read: "区分 SSE/重试/回退/Cache 与计划/动作 JSON 契约及错误路径。" },
    { area: "跟随一个 Goal turn", paths: ["internal/agent/agent.go", "internal/goal/store.go"], read: "观察决策、工具、Observation、Evaluation、预算和 Goal 转移如何互动。" },
